"""
Generates principal components of the encoding based on the output images produced by perturbing original prompt encoding by variational prompt encoding
"""
...
for i, p_var in enumerate(prompt_variations):
    ...
    outputs = []
    for vec_idx in range(var_text_outputs['xf_out'].shape[-1]):
        th.manual_seed(SEED)
