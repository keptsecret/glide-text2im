"""
Script for sampling images on perturbations based on singular vectors
"""
...
im = Image.open("./000000000034.png")
...
# pc = th.load("zebra_pca.pt")
# pc = th.load("airplane_pca.pt")
Vh = th.load("zebra_Vh.pt")
...
for i in range(Vh.shape[0]):
    ...
    scale_out = loss.item()
    dir_out = Vh[i].view(512, 128).to(device)
    dir_out = th.cat((dir_out.unsqueeze(0), zeros_mask.unsqueeze(0)), 0).to(th.float16)
    xf_out = text_outputs['xf_out'] + dir_out * global_scale #/ scale_out * alpha
    ...
    dir_out = Vh[i].view(512,128).to(device)
    dir_out = th.cat((dir_out.unsqueeze(0), zeros_mask.unsqueeze(0)), 0).to(th.float16)
    xf_out = text_outputs_up['xf_out'] + dir_out * global_scale #/ scale_out * alpha
    ...
    # Show the output
    show_images(up_samples[0], f'images/zebra_s{SEED}_vh{i}.png')
